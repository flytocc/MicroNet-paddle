...
class SwishLinear(nn.Layer):
    ...
    def __init__(self, inp, oup):
        super().__init__()
        self.linear = nn.Sequential(nn.Linear(inp, oup), nn.BatchNorm1D(oup),
                                    nn.Hardswish())
...
class GroupConv(nn.Layer):
    ...
    def __init__(self, inp, oup, groups=2):
        ...
        self.conv = nn.Sequential(
            nn.Conv2D(inp, oup, 1, groups=self.groups[0], bias_attr=False),
            nn.BatchNorm2D(oup))
    ...
    def __init__(self, inp, expand, kernel_size, stride):
        ...
        self.conv = nn.Sequential(
            nn.Conv2D(inp,
                      ...
                      (kernel_size // 2, 0),
                      groups=inp,
                      bias_attr=False), nn.BatchNorm2D(inp * exp1),
            nn.Conv2D(hidden_dim,
                      oup, (1, kernel_size), (1, stride),
                      (0, kernel_size // 2),
                      groups=hidden_dim,
...
class DYShiftMax(nn.Layer):
    ...
    def __init__(self,
        ...
        self.avg_pool = nn.Sequential(
            nn.ReLU() if relu_before_pool == True else nn.Identity(),
        ...
        self.fc = nn.Sequential(nn.Linear(inp, squeeze), nn.ReLU(),
    ...
    def __init__(self,
        ...
        if gs1[0] == 0:
            self.layers = nn.Sequential(
                ...
                ChannelShuffle2(hidden_dim2 //
                                2) if y2 != 0 else nn.Identity(),
            ...
            self.layers = nn.Sequential(
                ...
                ChannelShuffle2(hidden_dim2 //
                                4) if y1 != 0 and y2 != 0 else nn.Identity()
                ...
                DYShiftMax(oup,
                           ...
                           init_a=[init_ab3[0], 0.0],
                           reduction=act_reduction //
                           2 if oup < hidden_dim2 else act_reduction,
                           init_b=[init_ab3[1], 0.0],
                           g=(g1, g2),
                           expansion=False) if y3 > 0 else nn.Identity(),
                ChannelShuffle(g2),
...
class MicroNet(nn.Layer):
    ...
    def __init__(self,
        ...
        self.classifier = nn.Sequential(
            ...
            SwishLinear(out_ch, num_classes),
        )
...
def micronet_m0(**kwargs):
    ...
    return MicroNet(msnx_dy6_exp4_4M_221,
                    activation_cfg,
                    stem_ch=4,
                    stem_groups=[2, 2],
                    out_ch=640,
...
def micronet_m1(**kwargs):
    ...
    return MicroNet(msnx_dy6_exp6_6M_221,
                    activation_cfg,
                    stem_ch=6,
                    stem_groups=[3, 2],
                    out_ch=960,
...
def micronet_m2(**kwargs):
    ...
    return MicroNet(msnx_dy9_exp6_12M_221,
                    activation_cfg,
                    stem_ch=8,
                    stem_groups=[4, 2],
                    out_ch=1024,
...
def micronet_m3(**kwargs):
    ...
    return MicroNet(msnx_dy12_exp6_20M_020,
                    activation_cfg,
                    stem_ch=12,
                    stem_groups=[4, 3],
                    out_ch=1024,
